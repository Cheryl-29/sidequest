...
> 边界：persona 是合成的，不代表真实用户；上表只跑了开发集；3 次重复中三个记忆组逐项相同，无记忆组相差至多 1 个会话，但执行器的抽样种子是固定的，重复只覆盖模型采样的波动，不覆盖种子；保留集 persona 尚未用于正式实验。确定性的 `RuleModel` 替身（无网络、无模型）可以随时复现同一套机制：`typed_context` 首轮接受 28/44、提议 6/6、全同意 persona 错误记忆 0。
